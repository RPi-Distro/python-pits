...
static PyObject * BuildSentence_wrapper(PyObject * self, PyObject * args)
{
  char * TxLine;
  int SentenceCounter;
  struct TGPS * GPS;

  char FormatList[] = {"s", "i", "l", "l", "f", "f", "I", "I", "I", "i", "i", "f", "f", "f", "f", "f", "i", "f", "f"};

  // parse arguments
  if (!PyArg_ParseTuple(args, FormatList, &input, SentenceCounter, &GPS.Time, &GPS.Seconds, &GPS.Longitude, &GPS.Latitude, &GPS.Altitude, &GPS.MaximumAltitude, &GPS.Satellites, &GPS.Speed, &GPS.Direction, &&GPS.DS18B20Temperature, &GPS.BatteryVoltage, &GPS.BMP180Temperature, &GPS.Pressure, &GPS.BoardCurrent, &GPS.DS18B20Count, &GPS.PredictedLongitude, &GPS.PredictedLatitude)) {
    return NULL;
  }

  // run the actual function
  BuildSentence(TxLine, SentenceCounter, GPS);
...
// FIXME: Not sure how to convert speed_t to a python object???
static PyObject * BaudToSpeed_wrapper(PyObject * self, PyObject * args)
{
  int baud;
  speed_t result;
  PyObject * ret;
  ...
  // parse arguments
  if (!PyArg_ParseTuple(args, "i", baud){
    return NULL;
  }

  // run the actual function
  result = BaudToSpeed(baud);
...
static PyObject * SetMTX2Frequency_wrapper(PyObject * self, PyObject * args)
{
  char * FrequencyString;

  // parse arguments
  if (!PyArg_ParseTuple(args, "s", &FrequencyString)) {
    return NULL;
  }

  // run the actual function
  SetMTX2Frequency(FrequencyString);
}
...
static PyObject * SetNTX2BFrequency_wrapper(PyObject * self, PyObject * args)
{
  char * FrequencyString;
  ...
  // run the actual function
  SetNTX2BFrequency(FrequencyString);
}
...
static PyObject * OpenSerialPort_wrapper(PyObject * self)
{
  int fd;
  PyObject * ret;
...
static PyObject * SendSentence_wrapper(PyObject * self, PyObject * args)
{
  int fd;
  char * TxLine;

  // parse arguments
  if (!PyArg_ParseTuple(args, ["i", "s"], fd, &TxLine)) {
    return NULL;
  }

  // run the actual function
  SendSentence(fd, TxLine);
}
...
static PyObject * SendRTTYImage_wrapper(PyObject * self, PyObject * args)
{
  int fd;
  int result;
  PyObject * ret;
...
static PyObject * SendIPAddress_wrapper(PyObject * self, PyObject * args)
{
  int fd;

  // parse arguments
  if (!PyArg_ParseTuple(args, "s", fd)) {
    return NULL;
  }
...
static PyObject * SendFreeSpace_wrapper(PyObject * self, PyObject * args)
{
  int fd;
...
static PyObject * GetProgCount(PyObject * self, PyObject * args)
{
  int result;
  PyObject * ret;
  ...
  // test program count
  result = prog_count("tracker")

  // build the resulting string into a Python object.
  ret = PyString_FromFormat("%d", result);

  return ret;
}

static PyObject * GetNewBoard(PyObject * self, PyObject * args)
{
  int result;
  PyObject * ret;
...
static PyObject * SetAllConfig(PyObject * self, PyObject * args)
{
  int led_ok;
  int led_warn;
  int sda;
  ...
  // parse arguments
  if (!PyArg_ParseTuple(args, ["i", "i", "i", "i"], led_ok, led_warn, sda, scl)) {
    return NULL;
  }

  Config.LED_OK = led_ok;
  Config.LED_Warn = led_warn;

  Config.SDA = sda;
  Config.SCL = scl;
}

static PyObject * SetMonitor(PyObject * self, PyObject * args)
{
  // check if need to disable monitor and do it
...
static PyObject * SetCamera(PyObject * self, PyObject * args)
{
  // check if need set camera and do it
  LoadConfigFile(&Config);
  if (Config.Camera)
	{
...
static PyObject * SetGPS(PyObject * self, PyObject * args)
{
  // set GPS
  struct TGPS GPS;
  GPS.Time = 0.0;
